...
//	buffer= (char*) calloc (30,sizeof(char));
	...
	{	/// I am assuming that the type is (p,q) written as pq.
...
void programParams::announce()
{
		...
		std::cout<< "The  D^2 coupling is  "<< gD2<< "The  (Tr(D^2)^2 coupling is  "<< gD22<<"and the D^4 coupling is  "<< gD4 <<std::endl;
		if(measure==0)
		{
			std::cout <<"You are measuring set 0, that means only the action, kind of obsolete see action_monitor.txt" <<std::endl;
		}
		else if(measure==1)
		{
